...
mu          = 0.04;
sigr        = 0.2;
income_profile = 'stable';
market_profile = 'base';
output_root = '';
scenario_name = '';

if strcmp(income_profile,'stable')
   smay = 0.05;
   smav = 0.05;
elseif strcmp(income_profile,'volatile')
   smay = 0.10;
   smav = 0.20;
elseif strcmp(income_profile,'retired')
   tr = tb;
   smay = 0.0;
   smav = 0.0;
end
...
for t=1:tn-1
    if t<10
        filename{t} = ['year0' num2str(t)];
    else
        filename{t} = ['year' num2str(t)];
    end
end

if isempty(scenario_name)
   scenario_name = sprintf('rho_%02.0f_income_%s_market_%s',rho,income_profile,market_profile);
end

if isempty(output_root)
   output_dir = '.';
else
   output_dir = fullfile(output_root,scenario_name);
end
...
fileID = fopen(fullfile(output_dir,'metadata.json'),'w');
fprintf(fileID,'{\n');
fprintf(fileID,'  "scenario_name": "%s",\n',scenario_name);
fprintf(fileID,'  "rho": %.8f,\n',rho);
fprintf(fileID,'  "delta": %.8f,\n',delta);
fprintf(fileID,'  "psi": %.8f,\n',psi);
fprintf(fileID,'  "r": %.8f,\n',r);
fprintf(fileID,'  "mu": %.8f,\n',mu);
fprintf(fileID,'  "sigr": %.8f,\n',sigr);
fprintf(fileID,'  "income_profile": "%s",\n',income_profile);
fprintf(fileID,'  "market_profile": "%s",\n',market_profile);
fprintf(fileID,'  "tb": %d,\n',tb);
fprintf(fileID,'  "tr": %d,\n',tr);
fprintf(fileID,'  "td": %d,\n',td);
fprintf(fileID,'  "ncash": %d,\n',ncash);
fprintf(fileID,'  "na": %d,\n',na);
fprintf(fileID,'  "cash_grid_min": %.8f,\n',mincash);
fprintf(fileID,'  "cash_grid_max": %.8f\n',maxcash);
fprintf(fileID,'}\n');
fclose(fileID);
...
for i1 = 1:td-tr
   t = tn-i1;
    fileID = fopen(fullfile(output_dir,[filename{t} '.txt']),'w');
    for i5=1:ncash
...
fileID = fopen(fullfile(output_dir,'CWY.txt'),'w');
for i2=1:tn
...
fileID = fopen(fullfile(output_dir,'CWYs.txt'),'w');
for i2=1:tn
    fprintf(fileID,'%12.8f %12.8f %12.8f\r\n',meanCs(i2,1), meanWs(i2,1), meanYs(i2,1));
end
fclose(fileID);

fileID = fopen(fullfile(output_dir,'SB.txt'),'w');
for i2=1:tn
